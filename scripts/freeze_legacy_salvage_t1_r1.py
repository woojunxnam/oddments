...
from qa_common import DATA, dependency_snapshot, drug_consequence_rule_ids, load_json, load_records, question_audit_hash, semantic_content_hash, write_json

CANDIDATE_SHA = "6035cd92939bc094d0077b87d93ef8ac0b784415"
CANDIDATE_BRANCH = "repair/pre-batch3-legacy-salvage-t1-r1"
AUDITOR_INSTANCE = "GPT-FRESH-COV-T1-C"
DATE = "2026-08-17"
OUT = Path("audits/remediation/2026-08-17")
FREEZE_BRANCH = "freeze/pre-batch3-legacy-salvage-t1-r1-v1"
...
LEGAL_CONTRACT = {
    ...
    "pass_condition": "Verdict=KEEP and Existing_Answer_Correct=YES for changed-item admission; any other result requires adjudication before release.",
}
REALISM_CRITERIA = [
...
def main() -> int:
    ...
    blind_path = OUT / "GPT-C-BLIND-QUESTIONS-PRE-BATCH3-LEGACY-SALVAGE-T1-R1.json"
    blind = {
        ...
        "questions": [
            {"question_id": q["question_id"], "question_type": q.get("question_type"), "stem": q.get("stem"), "choices": q.get("choices")}
            for q in selected
        ],
        "content_boundary": "Intentionally excludes keyed answers, explanations, rule/drug links, lifecycle fields, family/editor classifications, and all prior-review conclusions so the auditor can solve independently first.",
    }
    ...
    common = {
        ...
        "content_boundary": "No question text, keyed answers, explanations, family/editor classifications, or prior-review conclusions are embedded in this contract package. Use the blind package for independent solving and verify canonical hashes mechanically.",
        "independence_note": "Independently solve all three changed questions before inspecting keyed answers, explanations, prior audit/editor/repair material, or repository conclusions. Current-law decisions must come from independently researched official sources.",
    }

    legal = {**common, "review_type": "LEGAL_VERIFICATION", "source_policy": "Current official primary/official sources only.", "result_contract": LEGAL_CONTRACT}
    realism = {
        **common,
    ...
    legal_path = OUT / "GPT-C-LEGAL-PRE-BATCH3-LEGACY-SALVAGE-T1-R1.json"
    realism_path = OUT / "GPT-C-REALISM-PRE-BATCH3-LEGACY-SALVAGE-T1-R1.json"
    ...
    manifest = {
        "manifest_type": "PRE_BATCH3_LEGACY_SALVAGE_T1_R1_CLEAN_FREEZE_V1",
        "created_date": DATE,
        ...
        "locked_next_step": "Fresh independent REAUDIT LEGAL_VERIFICATION + REALISM_REVIEW of exactly MA-Q-0028, MA-Q-0032, and MA-Q-0036 before repair admission.",
    }
    manifest_path = OUT / "PRE-BATCH3-LEGACY-SALVAGE-T1-R1-CLEAN-FREEZE-MANIFEST.json"
