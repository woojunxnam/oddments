"""Apply Batch 3 tranche B3-B: 31 rules and 33 questions, MA-Q-0262..MA-Q-0294.
...
from batch3b_questions import QUESTIONS
from batch3b_rules import RULES
from qa_common import DATA, load_json, load_records, semantic_content_hash, write_json


AUTHORING_DATE = "2026-08-19"
EXPECTED_IDS = [f"MA-Q-{index:04d}" for index in range(262, 295)]
...
def main() -> int:
    if [q["question_id"] for q in QUESTIONS] != EXPECTED_IDS:
        raise SystemExit("question IDs do not match the locked B3-B range MA-Q-0262..MA-Q-0294")
    ...
    for question in QUESTIONS:
        ...
        if family_id in saturated:
            raise SystemExit(f"{family_id} is already saturated; B3-B must not extend it")
        if family_id in known_families:
            raise SystemExit(f"{family_id} already exists; B3-B families must be new")
