...
GATE_ID = "PRE_BATCH3_FULL_COMPETENCY_COVERAGE_DEBT_POST_T2"
CONTROLLER_ISSUE = 83
GATE_ISSUE = 40
...
def build_manifest(bank: Bank, matrix: dict, matrix_path: Path) -> dict:
    ...
    return {
        "manifest_type": "POST_T2_PRE_BATCH3_FINAL_GATE_MANIFEST",
        "gate": GATE_ID,
        ...
        "frozen_source": {
            ...
            "role": "final post-T2 canonical release state audited by this gate",
        },
...
def main() -> int:
    ...
    matrix = {
        ...
        "gate_issue": GATE_ISSUE,
        "controller_issue": CONTROLLER_ISSUE,
        "representation": "FULL_FROM_SCRATCH_RECOMPUTATION_AT_EXACT_POST_T2_SHA",
    ...
    output = ROOT / "audits" / "coverage" / "2026-08-19" / "POST-T2-PRE-BATCH3-FINAL-COVERAGE-MATRIX.json"
    write_json(output, matrix)

    manifest_path = ROOT / "audits" / "coverage" / "2026-08-19" / "POST-T2-PRE-BATCH3-FINAL-GATE-MANIFEST.json"
