...
# Firewall: while an authority conflict is unresolved, a family whose legal proposition depends
# on the disputed meaning may not count toward guaranteed capacity.
VALID_CONFLICT_DEP = {"NONE", "Q0172_FIRST_DOSE_CONFLICT", "OTHER"}
...
REQUIRED_FIELDS = [
    ...
    "scenario_sketch", "viability", "authority_conflict_dependency",
]
...
def validate(records: list) -> list:
    ...
    for i, r in enumerate(records):
        ...
        if r.get("authority_conflict_dependency") not in VALID_CONFLICT_DEP:
            problems.append(f"{tag}: authority_conflict_dependency {r.get('authority_conflict_dependency')!r} "
                            f"not in {sorted(VALID_CONFLICT_DEP)}")
...
def summarize(records: list) -> dict:
    yes_all = [r for r in records if r.get("viability") == "YES"]
    # Firewall: only families independent of the unresolved authority conflict may be counted.
    yes = [r for r in yes_all if r.get("authority_conflict_dependency", "NONE") == "NONE"]
    conflict_held = [r for r in yes_all if r.get("authority_conflict_dependency", "NONE") != "NONE"]
    ...
    return {
        ...
        "viable_yes": len(yes),
        "viable_yes_including_conflict_held": len(yes_all),
        "held_by_authority_conflict": len(conflict_held),
        "held_family_ids": [r.get("proposed_family_id") for r in conflict_held],
        "rejected_no": len(no),
