"""Apply Batch 3 tranche B3-D: 21 rules and 33 Area-2 questions, MA-Q-0328..MA-Q-0360.
...
Unlike B3-A and B3-B, this tranche deliberately places TWO questions in twelve families. That is
...
from batch3d_questions import QUESTIONS
from batch3d_rules import RULES
from qa_common import DATA, load_json, load_records, semantic_content_hash, write_json


AUTHORING_DATE = "2026-08-20"
EXPECTED_IDS = [f"MA-Q-{index:04d}" for index in range(328, 361)]
FAMILY_CAP = 2
...
def main() -> int:
    if [q["question_id"] for q in QUESTIONS] != EXPECTED_IDS:
        raise SystemExit("question IDs do not match the locked B3-D range MA-Q-0328..MA-Q-0360")
    ...
    for question in QUESTIONS:
        ...
        if family_id in saturated:
            raise SystemExit(f"{family_id} is already saturated; B3-D must not extend it")
        if family_id in pre_existing:
            raise SystemExit(f"{family_id} already exists in the matrix; B3-D families must be new")
